...
import { Package, Search, AlertCircle, PhoneCall, Clock, CheckCircle2, Ban, RefreshCw, Phone } from 'lucide-react'
...
import { useCodVerificationsQuery, useCodVerificationStatsQuery } from '@/hooks/use-cod-verifications'
...
import { CodStatsCard } from '@/components/dashboard/cod-verifications/CodStatsCard'
...
export default function CodVerificationsPage() {
    ...
    const [selectedOrder, setSelectedOrder] = useState<CodVerificationItem | null>(null)
    ...
    // Fetch data from API
    const { data: listData, isLoading: isLoadingList, error: listError, refetch: refetchList, isFetching } = useCodVerificationsQuery(queryParams)
    const { data: statsData, isLoading: isLoadingStats, error: statsError } = useCodVerificationStatsQuery()
    ...
    function getVerificationStatus(item: CodVerificationItem): 'pending' | 'confirmed' | 'rejected' {
            ...
            <div className="flex-1 p-4 md:p-8 max-w-screen-2xl mx-auto w-full">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-5 md:mb-6">
                    {isLoadingStats ? (
                        <>
                            {[...Array(4)].map((_, i) => (
                                <div key={i} className="rounded-xl p-5" style={{ background: '#fff', border: '1px solid #f0f0f0' }}>
                        ...
                        <div className="col-span-4 p-5 rounded-xl" style={{ background: '#fff', border: '1px solid #f0f0f0' }}>
                            <p className="text-sm text-destructive">Failed to load stats. Please try again.</p>
                        </div>
                    ) : (
                        <>
                            <CodStatsCard
                            ...
                            <CodStatsCard
                                ...
                                icon={
                                    <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'var(--warning-container)' }}>
                    ...
                    {listError ? (
                        <div className="p-8 text-center">
                            ...
                            <p className="text-sm font-medium text-destructive mb-4">
                                Failed to load COD verifications: {listError instanceof Error ? listError.message : 'Unknown error'}
                            </p>
                            ...
                            <Table className="w-full">
                                <TableHeader>
                                    <TableRow className="bg-muted/50 hover:bg-muted/50">
                                        ...
                                        <TableHead className="py-4 px-6 text-left text-xs font-bold uppercase tracking-wider text-muted-foreground">Time</TableHead>
                                        <TableHead className="py-4 px-6 text-left text-xs font-bold uppercase tracking-wider text-muted-foreground">Customer</TableHead>
                                        <TableHead className="py-4 px-6 text-left text-xs font-bold uppercase tracking-wider text-muted-foreground">Phone</TableHead>
                                        ...
                                        <TableHead className="py-4 px-6 text-left text-xs font-bold uppercase tracking-wider text-muted-foreground">Amount</TableHead>
                                        ...
                                        <TableHead className="py-4 px-6 text-left text-xs font-bold uppercase tracking-wider text-muted-foreground">Action</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {isLoadingList ? (
                                            ...
                                            {Array.from({ length: 8 }).map((_, i) => (
                                                <TableCell key={i} className="py-4 px-6">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: ['120px', '100px', '180px', '120px', '100px', '100px', '120px', '100px'][i],
                                                        }}
                                                    />
                                                </TableCell>
                                            ))}
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="py-4 px-6">
                                                        <div>
                                                            ...
                                                            <p className="text-xs mt-0.5 text-muted-foreground">{item.user.email}</p>
                                                        </div>
                                                    </TableCell>
                                                    <TableCell className="py-4 px-6">
                                                        <a
